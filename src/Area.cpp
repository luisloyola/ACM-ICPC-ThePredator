...
int Area::recorrerArea(int i, int j, Celda** matrix, int nfilas, int ncolumnas, int valor_predator)
{
	int area_valor = 0;
	...
	{//si la celda no ha sido visitada y está a la altura del predator.
		...
		{//NO está en la fila superior por lo que se puede preguntar por la celda superior
			area_valor += this->recorrerArea(i-1, j, matrix, nfilas, ncolumnas, valor_predator);
		}
		...
		{//NO está en la columna izquierda por lo que se puede preguntar por la celda izquierda
			area_valor += this->recorrerArea(i, j-1, matrix, nfilas, ncolumnas, valor_predator);
		}
